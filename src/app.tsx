...
export default function App() {
  ...
  const clearData = () => {
    ...
    setCustomColConfigs([]);
    setActiveTab("trangchu"); // Quay lại trang chủ khi xóa dữ liệu
  };
  ...
  const filteredData = useMemo(() => {
    ...
    return mainData.filter(row => {
      // Tìm kiếm trên tất cả các cột của dòng hiện tại
  ...
  const handleMerge = async () => {
    ...
    // Auto map idCol nếu nó chưa được map
    if (!mapping.idCol) {
  ...
  const handleQuickReport = async (level: number) => {
    ...
    const processedData = mainData.map(row => {
      ...
      const mng = normalizeSectorCode(mngRaw); // Mã ngành cấp 5 đã chuẩn hóa
      
      // Lấy thông tin cấp 5, cấp 2, cấp 1 từ mã ngành chuẩn hóa
      const hierarchy = getSectorHierarchy(mng);
      const cap5Info = hierarchy["5"];
      const cap2Info = hierarchy["2"];
      const cap1Info = hierarchy["1"];

      // Tên ngành Cấp 5, Cấp 2, Cấp 1 đã được định dạng chuẩn
      ...
      // Tạo nhãn ngành dựa trên cấp độ yêu cầu (level 1 hoặc 2)
      let tenNganhLabel = ""; // Nhãn sẽ dùng để gom nhóm
      if (level === 2) {
        // Lấy 2 số đầu của mã ngành cấp 5 đã chuẩn hóa để làm mã Cấp 2
      ...
      // Trả về dòng dữ liệu gốc kèm theo các thông tin ngành đã phân tích
      return {
        ...row,
        ...
        _tenNganhLabel: tenNganhLabel, // Nhãn ngành hiển thị cho cấp độ báo cáo (dùng để gom nhóm)
    ...
    if (reportType === "pivot") {
      // Báo cáo xoay Pivot: Xã là dòng, các ngành và chỉ số là cột
      setStatusMessage("Đang tiến hành xoay (Pivot) gom nhóm theo từng Ngành Kinh Tế làm cột...");
      await sleep(150);

      // Lấy danh sách duy nhất các Xã và các Nhãn Ngành đã tạo
      const communes = Array.from(new Set(processedData.map(r => r._tempXa))).sort();
      const sectorLabels = Array.from(new Set(processedData.map(r => r._tenNganhLabel))).sort(); // Sử dụng nhãn ngành đã tạo làm key

      // Duyệt qua từng Xã để tạo dòng báo cáo
      communes.forEach((commune, cIdx) => {
        const communeObj: any = {
          "Địa_Bàn_Xã": commune // Cột đầu tiên là tên Xã
        };
        ...
        // Duyệt qua từng Nhãn Ngành để tính toán chỉ số cho Xã đó
        sectorLabels.forEach(sectorLabel => {
          ...
          // Tính tổng Doanh Thu và Lao Động cho các dòng khớp ngành
          ...
          // Tên cột được tạo động: "[Nhãn Ngành] - Tổng Doanh Thu" & "[Nhãn Ngành] - Tổng Lao Động"
          ...
          totalCommuneDN += matchedRows.length; // Tổng số DN cho Xã
          totalCommuneDoanhThu += sumDoanhThu; // Tổng DT cho Xã
          totalCommuneLaoDong += sumLaoDong; // Tổng LĐ cho Xã
        });

        // Thêm cột tổng kết cho Xã
      ...
      // Báo cáo dạng phẳng (flat): Gom nhóm theo các tiêu chí ngành và xã
      const groups = new Map<string, any[]>();
      processedData.forEach(row => {
        // Sử dụng các trường ngành và xã đã chuẩn hóa để tạo khóa gom nhóm
      ...
      keys.forEach(keyStr => {
        const dims = JSON.parse(keyStr); // Các thuộc tính dùng để gom nhóm
        const rowsObj = groups.get(keyStr) || []; // Các dòng dữ liệu thuộc nhóm này

        let sumDoanhThu = 0;
        let sumLaoDong = 0;

        // Tính tổng DT và LĐ cho nhóm
        rowsObj.forEach(r => {
          if (mapping.doanhthu) {
            const val = parseFloat(String(r[mapping.doanhthu]).replace(/[^0-9.\-]/g, ""));
            if (!isNaN(val)) sumDoanhThu += val;
        ...
        // Tạo dòng báo cáo phẳng, hiển thị tên ngành theo cấp độ báo cáo yêu cầu
        finalReportRows.push({
          [`Ngành_Cấp_${level}`]: level === 1 ? dims.Ngành_Cấp_1 : dims.Ngành_Cấp_2, // Tên ngành Cấp 1 hoặc Cấp 2 tùy theo lựa chọn
          ...
          "Số_Lượng_Doanh_Nghiệp": rowsObj.length, // Số lượng DN trong nhóm
  ...
  const handleStandardizeSectors = async (useAI: boolean) => {
    ...
    for (let index = 0; index < mainData.length; index++) {
      ...
      const maDtvVal = normalizeSectorCode(maNganhRaw); // Chuẩn hóa mã ngành (thành cấp 5 hoặc cấp 2/1 tùy theo đầu vào)

      // Phân cấp mã ngành ĐTV đăng ký
      const hier = getSectorHierarchy(maDtvVal);
      const cap1Info = hier["1"];
      const cap2Info = hier["2"];
      // const cap3Info = hier["3"]; // Không dùng đến trong logic này
      // const cap4Info = hier["4"]; // Không dùng đến trong logic này
      ...
      let giaiThich = "";
      let linhvucSuggest = ""; // Lĩnh vực cấp 1 gợi ý từ AI

      if (useAI) {
        // Thực hiện cuộc gọi Gemini API Server proxy (giả định có API endpoint)
        setStatusMessage(`[Phân tích AI] Đang dịch nghĩa dòng ${index + 1}/${mainData.length}: "${motaVal.slice(0, 30)}..."`);
        try {
          // Giả sử bạn có một API endpoint `/api/gemini/analyze` để xử lý
          ...
          if (data && !data.error) {
            goiyMa = normalizeSectorCode(data.goiy_ma); // Chuẩn hóa mã gợi ý
            goiyTen = data.goiy_ten || "";
            linhvucSuggest = data.cap_1_tin_cay || ""; // Lĩnh vực gợi ý từ AI (Cấp 1)
            giaiThich = data.giai_thich || "Phân tích từ AI";
            diemTuongDong = "0.95"; // Ước lượng độ tin cậy AI (có thể lấy từ API nếu có)
          } else {
            ...
            if (local) {
              ...
              linhvucSuggest = getSectorHierarchy(goiyMa)["1"]?.ma || ""; // Lĩnh vực cấp 1 từ mã gợi ý
            }
          }
        } catch (e) {
          // Fall back nếu có lỗi mạng hoặc API không khả dụng
        ...
        if (local) {
          ...
          // Lấy lĩnh vực cấp 1 từ mã gợi ý
          const sugHier = getSectorHierarchy(goiyMa);
          linhvucSuggest = sugHier["1"]?.ma || "";
        }
      }

      // Đọc tên ngành Cấp 5 từ vsicRawData dựa trên mã ngành ĐTV đã chuẩn hóa
      const stdCap5Ten = vsicRawData[maDtvVal] || `[Không tìm thấy tên Cấp 5 cho mã ${maDtvVal}]`;
      
      // Lấy tên ngành Cấp 2 và Cấp 1 đã chuẩn hóa từ hierarchy
      ...
      // Đánh giá Trạng thái logic khớp mã ngành ĐTV và Mô tả hoạt động
      let trangThai = "✅ Hợp lệ"; // Mặc định là hợp lệ
      
      // Lấy mã lĩnh vực cấp 1 từ mã ngành đăng ký (ĐTV)
      const dtvLinhVuc = cap1Info?.ma || ""; // Mã lĩnh vực cấp 1 của mã ngành ĐTV

      // Phân tích đối chiếu tương đồng và phát hiện sai lệch thực tế theo mẫu ĐTV ghi
      const lcMota = motaVal.toLowerCase(); // Mô tả hoạt động chuyển thành chữ thường
      
      // Các từ khóa để nhận diện hoạt động Thương mại/Bán lẻ
      const hasTradeKeywords = ["bán", "mua", "thương mại", "đại lý", "cửa hàng", "phân phối", "bán lẻ", "kinh doanh", "buôn bán", "nhà buôn", "tiệm", "shop", "market", "sales", "trade", "retail", "wholesale"];
      // Các từ khóa để nhận diện hoạt động Sản xuất/Công nghiệp
      const hasIndustrialKeywords = ["sản xuất", "gia công", "chế tạo", "chế biến", "lắp ráp", "cơ khí", "dệt", "may", "in ấn", "nhà máy", "xưởng", "công nghiệp", "sản phẩm", "hàng hóa", "manufacture", "produce", "factory", "workshop", "industrial", "processing", "crafting"];

      // Xác định mã ngành có phải là Công nghiệp hay không
      const activeSub2 = maDtvVal ? maDtvVal.slice(0, 2) : "";
      const sub2Num = parseInt(activeSub2, 10);
      // Mã ngành Công nghiệp bao gồm: Cấp 2 từ 10-33 HOẶC cấp 1 là C (Công nghiệp chế biến chế tạo)
      const isIndustrialCode = (!isNaN(sub2Num) && sub2Num >= 10 && sub2Num <= 33) || dtvLinhVuc === "C";

      // Logic kiểm tra và gán trạng thái lỗi/cảnh báo
      if (!vsicRawData[maDtvVal]) { // Kiểm tra xem mã ngành ĐTV có tồn tại trong danh mục VSIC chuẩn không
        trangThai = "❌ Lỗi: Mã ngành ĐTV không tồn tại trên danh mục VSIC chuẩn";
      } else if (hasTradeKeywords.some(kw => lcMota.includes(kw)) && !hasIndustrialKeywords.some(kw => lcMota.includes(kw)) && isIndustrialCode) {
        // Phát hiện: Mô tả hoạt động nghiêng về Thương mại nhưng Mã đăng ký lại thuộc nhóm Công nghiệp
        trangThai = "❌ Lỗi: Mô tả KD nghiêng về TM/Bán lẻ nhưng Mã đăng ký lại thuộc nhóm Công nghiệp";
      } else if (hasIndustrialKeywords.some(kw => lcMota.includes(kw)) && !isIndustrialCode && dtvLinhVuc !== "C") {
        // Phát hiện: Mô tả hoạt động nghiêng về Sản xuất/Công nghiệp nhưng Mã đăng ký lại thuộc nhóm ngoài Công nghiệp
        ...
        // Phát hiện: Mã ngành gợi ý có lĩnh vực khác với mã ngành đăng ký
        trangThai = `❌ Lỗi (LỆCH LĨNH VỰC): Mô tả KD thiên về Nhóm [${linhvucSuggest}] nhưng Mã đăng ký thuộc Nhóm [${dtvLinhVuc}]`;
      } else if (goiyMa && parseFloat(diemTuongDong) > 0.6) { // Nếu có gợi ý từ AI/Matcher với độ tin cậy cao
        // Kiểm tra xem phân nhóm cấp 2 đăng ký có lệch với gợi ý không
        const regCap2 = cap2Info?.ma || ""; // Mã cấp 2 của mã ngành ĐTV
        const sugHier = getSectorHierarchy(goiyMa); // Hierarchy của mã ngành gợi ý
        const sugCap2 = sugHier["2"]?.ma || ""; // Mã cấp 2 của mã ngành gợi ý
      ...
      // Thêm các cột mới vào kết quả
      standardizedResults.push({
        ...row, // Giữ lại các cột gốc
        "Mã_Ngành_Đăng_Ký_Chuẩn": maDtvVal, // Mã ngành ĐTV đã chuẩn hóa (có thể là cấp 5, 4, 3, 2, 1)
        "Tên_Ngành_Cấp_5_VSIC": stdCap5Ten, // Tên ngành Cấp 5 theo VSIC
        "Mã_Ngành_Cấp_2": cap2Info?.ma || "", // Mã ngành Cấp 2 (nếu có)
        "Tên_Ngành_Cấp_2": stdCap2Ten, // Tên ngành Cấp 2 (nếu có)
        "Mã_Ngành_Cấp_1": cap1Info?.ma || "", // Mã ngành Cấp 1
        "Tên_Ngành_Cấp_1": stdCap1Ten, // Tên ngành Cấp 1
        "Trạng_Thái_Kiểm_Tra_VSIC": trangThai, // Trạng thái kiểm tra (Hợp lệ/Lỗi/Cảnh báo)
        // --- Các cột gợi ý từ AI/Matcher ---
        "Mô_Tả_Hoạt_Động_Gốc": motaVal, // Mô tả hoạt động gốc (hiển thị lại để dễ so sánh)
        "Mã_Ngành_Gợi_Ý_AI": goiyMa, // Mã ngành gợi ý từ AI/Matcher
        "Tên_Ngành_Gợi_Ý": goiyTen, // Tên ngành gợi ý
        "Độ_Tin_Cậy_Matcher": diemTuongDong, // Điểm tin cậy của gợi ý
        "Giải_Thích_Phân_Tích": giaiThich, // Mô tả giải thích từ AI/Matcher
        "Lĩnh_Vực_KD_Gợi_Ý_C1": linhvucSuggest // Lĩnh vực cấp 1 gợi ý
      });

      // Chỉ nghỉ ngắn để UI giữ responsive và mượt mà, tránh block luồng chính
  ...
  const handleLogicRuleAdd = (type: "if" | "then") => {
    if (type === "if") {
      ...
      setNewIfRule({ col: "", op: "==", val: "" }); // Reset form
    } else {
      if (!newThenRule.col) {
      ...
      setNewThenRule({ col: "", op: "==", val: "" }); // Reset form
    }
  };
  ...
  const handleLogicCheck = async () => {
    ...
    // Hàm kiểm tra giá trị theo toán tử
    const checkValue = (rowVal: any, op: string, compareVal: string) => {
      ...
      // Kiểm tra dạng rỗng hoặc không rỗng
      if (op === "trống") return v1 === "";
      if (op === "không trống") return v1 !== "";

      // Kiểm tra dạng số (chuyển đổi và xử lý NaN)
      const num1 = parseFloat(v1.replace(/[^0-9.\-]/g, "")); // Loại bỏ ký tự không phải số
      const num2 = parseFloat(v2.replace(/[^0-9.\-]/g, ""));

      // Nếu cả hai là số hợp lệ, thực hiện so sánh số
      ...
      // Nếu không phải số, thực hiện so sánh chuỗi
      if (op === "==") return v1 === v2;
      if (op === "!=") return v1 !== v2;
      if (op === "chứa") return v1.includes(v2);
      if (op === "không chứa") return !v1.includes(v2);
      
      return false; // Trường hợp không khớp toán tử hoặc giá trị
    };

    const results = mainData.map((row, index) => {
      // 1. Đánh giá điều kiện NẾU (IF condition)
      const ifMatches = ifRules.map(r => checkValue(row[r.col], r.op, r.val));
      const satisfiesIf = ifCombine === "AND" 
        ? ifMatches.every(v => v === true) // Tất cả IF phải đúng
        : ifMatches.some(v => v === true); // Chỉ cần 1 IF đúng

      // 2. Đánh giá điều kiện THÌ PHẢI (THEN condition)
      const thenMatches = thenRules.map(r => checkValue(row[r.col], r.op, r.val));
      const satisfiesThen = thenCombine === "AND"
        ? thenMatches.every(v => v === true) // Tất cả THEN phải đúng
        : thenMatches.some(v => v === true); // Chỉ cần 1 THEN đúng

      let biViPham = false; // Cờ báo vi phạm logic
      let noteLoi = ""; // Ghi chú chi tiết lỗi

      // Logic kiểm tra: Nếu điều kiện NẾU thỏa mãn MÀ điều kiện THÌ PHẢI KHÔNG thỏa mãn -> Vi phạm
      if (satisfiesIf && !satisfiesThen) {
        biViPham = true;
        // Tạo mô tả chi tiết cho lỗi
      ...
      // Lấy lỗi logic cũ (nếu có) để append vào
      const existingLoi = String(row["Loi_Logic"] || "");

      // Trả về dòng dữ liệu mới, cập nhật cột "Loi_Logic"
      return {
        ...row,
        "Loi_Logic": biViPham 
          ? (existingLoi ? existingLoi + noteLoi : noteLoi) // Nếu có lỗi mới và đã có lỗi cũ, nối vào
          : (existingLoi || "✅ Đạt") // Nếu không có lỗi mới, giữ nguyên lỗi cũ (nếu có) hoặc ghi "Đạt"
      };
    });

    setMainData(results);
    setColumns(Object.keys(results[0] || {}));
    setFileName(`KiemTraLogic_${fileName}`); // Đổi tên file kết quả
  ...
  const handleExportExcel = () => {
    ...
    setTimeout(() => {
      try {
        ...
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Bao_Cao_Ket_Qua"); // Đặt tên sheet
        XLSX.writeFile(wb, fileName || "Ket_Qua_Xu_Ly_Du_Lieu.xlsx"); // Tên file mặc định nếu chưa có
    ...
    }, 200); // Delay nhỏ để UI kịp cập nhật trạng thái
  };
                  ...
                  <div className="bg-[#111827]/90 rounded-2xl p-5 border border-purple-500/20 space-y-5 animate-slide-up">
                    <div className="border-b border-gray-800 pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2">
                        <button 
                          onClick={() => {
                            ...
                            const resetConfigs = customColConfigs.map(c => ({
                              ...c,
                              use: true, // Reset 'use' to true
                              newName: c.originalName // Reset 'newName' to 'originalName'
                            }));
                        ...
                        <button 
                          onClick={() => {
                            // Auto fill nice names based on common patterns
                            const prefilled = customColConfigs.map(cfg => {
                              const c = cfg.originalName;
                              let newN = c; // Default to original name
                              ...
                              // Add more common mappings here if needed
                                ...
                                <td className="p-3">
                                  <select
                                    ...
                                    onChange={(e) => {
                                      ...
                                      // Avoid assigning the same unique role to multiple columns
                                      if (val !== "") {
                                        updated.forEach((c, cIdx) => {
                                          if (cIdx !== idx && c.role === val) {
                                            c.role = ""; // Reset the role of the previously assigned column
                                          }
                                        });
                                      }
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-[#374151] pb-4">
                    <div className="relative w-full sm:max-w-xs">
                      ...
                      <input 
                        ...
                        value={searchTerm}
                        onChange={(e) => { setSearchTerm(e.target.value); setViewPage(1); }} // Reset page khi tìm kiếm
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="bg-[#1f2937] border border-[#374151] rounded-2xl p-6 space-y-4">
                ...
                {oldData.length > 0 && newData.length > 0 && (
                  <div className="max-w-md space-y-1 bg-[#111827] rounded-xl p-4 border border-[#374151] mx-auto">
